...
import { getVoiceChannel, getGuildAndShoukaku, getTrack, addToFront, setupPlayerQueueListener, sendError, sendMessage, type Track } from "../../utils";
...
class PlayNowCommand implements Command {
  readonly title = "PlayNow";
  readonly description = "Play a song immediately";
  ...
  async execute(
    ...
    // 현재 재생 중인지 확인
    const isPlaying =
      (player as { track?: any }).track !== undefined &&
      (player as { track?: any }).track !== null;

    try {
      if (isPlaying) {
        // 재생 중이면 현재 트랙을 큐 앞에 추가하고 새 트랙 재생
        const currentTrack = (player as any)._currentTrack as Track | undefined;
        if (currentTrack?.encoded) {
          addToFront(guild.id, currentTrack);
        }
        // 현재 재생 중지
